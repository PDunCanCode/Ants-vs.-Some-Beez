...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        within_range = 0
        nearest_bee = None
        while nearest_bee is None and current_place != hive and within_range <= self.max_range :
            if len(current_place.bees) == 0 or within_range < self.min_range:
                current_place = current_place.entrance
                within_range += 1
            else:
                nearest_bee = current_place
        if nearest_bee == None:
            return None
        else: 
            return random_or_none(nearest_bee.bees)
        # END Problem 3 and 4
